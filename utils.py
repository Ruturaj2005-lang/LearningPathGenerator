...
async def setup_agent_with_tools(
    ...
    calendar_pipedream_url: Optional[str] = None,
    ...
    try:
        ...
        if calendar_pipedream_url:
            tools_config["calendar"] = {
                "url": calendar_pipedream_url,
                "transport": "streamable_http"
            }
            if progress_callback:
...
def run_agent_sync(
    ...
    async def _run():
        agent = await setup_agent_with_tools(
            ...
            calendar_pipedream_url=calendar_pipedream_url,
        ...
        # Append calendar scheduling instruction to prompt
        learning_path_prompt = (
            f"User Goal: {user_goal}\n"
            f"{user_goal_prompt}\n"
            f"Now schedule these learning sessions as events in Google Calendar. "
            f"Make sure to include time, title, and a short description for each day."
        )
